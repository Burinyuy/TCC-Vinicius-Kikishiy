...
def poseCallback(pose_message):
    ...
    #recebe a orientação do rbo em quaternios
...
def gripper(state,side):
    ...
    if state=="open": #parametros para abrir a garra
        ...
        point.positions=[0.038,0.038]#0.01,0.01
    else:
        print("ERROR")
...
def pick(velocity_publisher, start):
    ...
    for i in range(2):
        ...
        move_to_goal(velocity_publisher,sink[0],sink[1],sink[2]) #define e posição do próximo percurso
...
def place(start):
    ...
    for i in range(2):
        ...
        print("Second task finnished")

def task(start):
    global dinner_table, purchase_table, sink, counter, home
    if start=="go":
        print('start operation') #inicia a tarefa
        move_to_goal(velocity_publisher,dinner_table[0],dinner_table[1],dinner_table[2])#define e primeira posição do percurso
